...
export async function GET(request: NextRequest) {
  ...
  const result: any = {
    siteName: config.SiteConfig.SiteName, // Changed to lowercase 's' as per instruction
    announcement: config.SiteConfig.Announcement,
    doubanProxyType: config.SiteConfig.DoubanProxyType,
    doubanImageProxyType: config.SiteConfig.DoubanImageProxyType,
    // 🔥 性能优化：添加新的配置选项
    homePageConfig: {
      enableHeroBanner: config.HomePageConfig?.enableHeroBanner ?? true, // 默认启用（向后兼容）
    },
    danmuConfig: {
      enabled: config.DanmuConfig?.enabled ?? true,
    },
    playerEnhancementConfig: {
      enableAnime4K: config.PlayerEnhancementConfig?.enableAnime4K ?? true,
    },
