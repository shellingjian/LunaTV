...
export async function configSelfCheck(
  ...
  if (!adminConfig.NetDiskConfig) {
    adminConfig.NetDiskConfig = {
      enabled: false, // 🔥 默认禁用（性能优化）
      pansouUrl: 'https://so.252035.xyz', // 默认公益服务
      timeout: 30, // 默认30秒超时
      enabledCloudTypes: [], // 🔥 默认清空（性能优化）
    };
  }
  ...
  // 确保ACG搜索配置有默认值
  if (!adminConfig.ACGSearchConfig) {
    adminConfig.ACGSearchConfig = {
      enabled: false, // 🔥 默认禁用（性能优化）
    };
  }

  // 确保播放器增强配置有默认值
  if (!adminConfig.PlayerEnhancementConfig) {
    adminConfig.PlayerEnhancementConfig = {
      enableAnime4K: false, // 🔥 默认禁用（性能优化）
    };
  }
  ...
  if (!adminConfig.HomePageConfig) {
    adminConfig.HomePageConfig = {
      enableHeroBanner: false, // 🔥 默认禁用（性能优化）
    };
  }
